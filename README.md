...
- Terminate(): 종료 함수이며, Initialize(=초기화) 함수로 확보한 스택을 해제.
------------------------------------------------------------------------------------
## 큐(Que) 관련 용어
...
## 카운트 정렬(count)
- 주어진 배열 a를 스캔하며 원소 값이 배열 f의 인덱스와 일치하는 곳에 카운트를 올리며 도수분포표를 채운다.

## 트리(Tree)란?
- 트리를 구성하는 요소는 노드(node)와 가지(edge) 두가지 요소로 나뉘어져있다.
- 각각의 노드는 가지를 통해 다른 노드와 연결되어 있고, 다른 노드의 종류는 루트 노드와 단말 노드 2가지로 또 나뉘어진다

- 트리의 크기: 루트 포함한 모든 자손 노드의 개수
- 트리의 깊이: 루트에서 어떤 노드에 도달하기 위해 거쳐야하는 간선의수
- 트리의 레벨: 트리의 특정 깊이를 가지는 노드의 집합
- 트리의 차수: 각 노드가 지닌 가지의 수
- 트리의 차수2: 트리의 최대 차수
- 트리의 높이: 루트 노드에서 가장 깊숙이 있는 노드의 깊이
- 서브 트리: 트리 안에서 다시 어떤 노드를 루트로 정하고 그 자손으로 이루어진 트리
- 널 트리: 노드,가지가 없는 트리를 뜻한다.

## 이진 검색 트리
- 이진 검색 트리의 특징

- 구조가 단순하다.
- 중위 순회를 하면 키 값의 오름차순으로 노드를 얻을 수 있다.
